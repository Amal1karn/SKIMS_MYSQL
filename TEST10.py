...
class Generate_report_class:
    def __init__(self, root, enable_button):
        ...
        self.preview_text = scrolledtext.ScrolledText(self.manage_frame, state=tk.NORMAL, height=25, width=100, wrap=tk.WORD)
    ...
    def load_data_from_database(self):
        ...
        try:
            self.connection = sqlite3.connect('SerialTracking.db')
        ...
        for table_name in ["FM_SubsystemTest", "EDM_SubsystemTest"]:
            try:
                self.cursor.execute(f"SELECT DISTINCT System FROM {table_name};")
                system_values = self.cursor.fetchall()
                ...
                for system_value in system_values:
                    ...
                    self.cursor.execute(f"SELECT DISTINCT Subsystem_SN FROM {table_name} WHERE System = ?;",
                                        (system_name,))
                    subsystem_values = self.cursor.fetchall()
                    ...
                    try:
                        ...
                        if len(columns) > 2:
                            for column_index in range(2, len(columns)):
                                ...
                                self.cursor.execute(
                                    f"SELECT DISTINCT {column_name} FROM {table_name} WHERE System = ? AND Subsystem_SN = ?;",
                    ...
                    for value in distinct_values:
                        ...
                        if len(columns) > 3:  # Make sure there's a next following column
                            ...
                            self.cursor.execute(
                                ...
                                (value[0],))
                            next_distinct_values = self.cursor.fetchall()
                                ...
                                if child_key not in unique_nodes:
                                    ...
                                    if len(columns) > 3:  # Make sure there's a next following column
                                        ...
                                        self.cursor.execute(
                                            ...
                                            (next_value[0],))
                                        next_granddistinct_values = self.cursor.fetchall()
    ...
    def show_preview(self, event):
        ...
        for table_name in tables:
            self.cursor.execute(f"SELECT * FROM {table_name} WHERE Subsystem_SN = ?;", (item_text,))
        ...
        try:
            ...
            self.preview_text.insert(tk.END, f" _________________________________________________________________\n")
            self.preview_text.insert(tk.END, f"||________________________________________________________________||\n")
            ...
            self.preview_text.insert(tk.END, f"||\tComments\t\t:   {fetched_data[7]}\n")
            self.preview_text.insert(tk.END, f"||\tReport Link\t\t:   {fetched_data[8]}\n")
            self.preview_text.insert(tk.END, f"||________________________________________________________________\n")
            self.preview_text.insert(tk.END, f"||________________________________________________________________||\n")
            self.preview_text.tag_configure("gray", foreground="black")
            self.preview_text.tag_add("gray", "1.0", "end-1c linestart+1c")
        ...
        self.preview_text.tag_configure("colored", foreground="red", font=("Helvetica", 10, "bold"))
        self.preview_text.tag_add("colored", "1.0", "end-1c linestart+1c lineend")
        self.preview_text.tag_configure("bold", font=("Helvetica", 15, "bold"))
        self.preview_text.tag_add("bold", "1.0", "end")
    ...
    def tab_satellite_Integration_fm(self):
        ...
        custom_style.configure("Custom.TNotebook.Tab", background="white", fg="blue",
        ...
        def populate_tabs():
            cursor.execute("SELECT DISTINCT Satellite_SN FROM FM_SatelliteIntegration")
            ...
            for satellite_sn in satellite_sns:
                ...
                cursor.execute("SELECT DISTINCT System FROM FM_SatelliteIntegration WHERE Satellite_SN=?",
                               (satellite_sn[0],))
                ...
                for system in systems:
                    ...
                    cursor.execute("SELECT Subsystem_SN FROM FM_SatelliteIntegration WHERE Satellite_SN=? AND System=?",
                    ...
                    self.full_report_button = tk.Button(subsystem_frame, text="Report", font=("arial", 9, "bold"),
                                                        relief=RAISED, bd=5, bg="#0F148C", cursor="hand2", fg="white",
                                                        width=5, command=self.show_full_report_fm)
        ...
        self.fm_preview_text = scrolledtext.ScrolledText(self.sifm,wrap=tk.WORD)
    ...
    def show_satellite_fm_preview(self, subsystem_sn):
        ...
        for table_name in tables:
            cursor.execute(f"SELECT * FROM {table_name} WHERE Subsystem_SN = ?;", (subsystem_sn,))
        ...
        if fetched_data:
            ...
            self.fm_preview_text.insert(tk.END, f" _________________________________________________________________\n")
            self.fm_preview_text.insert(tk.END,
                                        f"||________________________________________________________________||\n")
            self.fm_preview_text.insert(tk.END, f"||\tTested Date\t\t:   {fetched_data[1]}\n")
            self.fm_preview_text.insert(tk.END, f"||\tSystem\t\t:   {fetched_data[2]}\n")
            self.fm_preview_text.insert(tk.END, f"||\tSerial No.\t\t:   {fetched_data[3]}\n")
            ...
            self.fm_preview_text.insert(tk.END, f"||\tComments\t\t:   {fetched_data[7]}\n")
            ...
            self.fm_preview_text.insert(tk.END,
                                        f"||________________________________________________________________||\n")
            self.fm_preview_text.tag_configure("red", foreground="black")
            self.fm_preview_text.tag_add("red", "1.0", "end-1c linestart+1c")
        else:
            ...
            self.fm_preview_text.insert(tk.END,
                                        f"||________________________________________________________________||\n")
            self.fm_preview_text.tag_configure("colored", foreground="red", font=("Helvetica", 10, "bold"))
            self.fm_preview_text.tag_add("colored", "1.0", "end-1c linestart+1c lineend")

        self.fm_preview_text.tag_configure("bold", font=("Helvetica", 13, "bold"))
        self.fm_preview_text.tag_add("bold", "1.0", "end")
    ...
    def show_full_report_fm(self):
        ...
        cursor.execute("SELECT DISTINCT System FROM FM_SatelliteIntegration WHERE Satellite_SN=?", (satellite_sn,))
        systems = cursor.fetchall()

        self.fm_preview_text.config(state=tk.NORMAL)
        self.fm_preview_text.delete(1.0, tk.END)

        self.fm_preview_text.insert(tk.END, f"\t\tReport for SATELLITE : {satellite_sn}\n")
        self.fm_preview_text.insert(tk.END, f"\t\t___________________________________________\n\n")
        self.fm_preview_text.insert(tk.END, "   SYSTEMS:\n")
        self.fm_preview_text.insert(tk.END, f"   ______________________________________________________________________________________\n")
        for system in systems:
            self.fm_preview_text.insert(tk.END, f"\t*{system[0]}\n")

            cursor.execute(
                "SELECT Subsystem_SN, Tested_Date, Tested_By, Comments FROM FM_SatelliteIntegration WHERE Satellite_SN=? AND System=?",
            ...
            for subsystem in subsystems:
                ...
                self.fm_preview_text.insert(tk.END, f"\t\tComments\t\t: {subsystem[3]}\n")
                # self.fm_preview_text.insert(tk.END, f"\t\t---------------------------\n")
                self.fm_preview_text.insert(tk.END, f"\t\t___________________________\n")

        self.fm_preview_text.tag_configure("thick_border", borderwidth=3, relief="solid")
        self.fm_preview_text.tag_add("thick_border", "1.0", "end")
        # Define a tag for different font styles
        self.fm_preview_text.tag_configure("italic", font=("Helvetica", 17, "bold"))
        # Apply the "italic" tag to a specific range of text
        self.fm_preview_text.tag_add("italic", "1.0", "1.100")

        # Apply the "bold" tag to a different range of text
        self.fm_preview_text.tag_configure("bold", font=("Helvetica", 14, "bold"), foreground="red", background="white")
        self.fm_preview_text.tag_add("bold", "3.0", "3.100")
        # Configure and apply color tags similarly
        self.fm_preview_text.tag_configure("red_text", foreground="red")
        self.fm_preview_text.tag_add("red_text", "3.0", "3.50")

        self.fm_preview_text.tag_configure("colored", foreground="red", font=("Helvetica", 10, "bold"))
        self.fm_preview_text.tag_add("colored", "5.0", "end-5c linestart+5c lineend")
    ...
    def tab_satellite_Integration_edm(self):
            ...
            for satellite_sn in satellite_sns:
                ...
                for system in systems:
                    ...
                    for subsystem_sn in subsystem_sns:
                        subsystem_label = ttk.Label(subsystem_frame, text=f"Subsystem: {subsystem_sn[0]}",
                                                    cursor="hand2",
                                                    font=("Arial", 12, "bold"), foreground="white", background="#20283E")
                        subsystem_label.bind("<Button-1>",
        ...
        # Create a custom style
        custom_style = ttk.Style()
        custom_style.configure("Custom.TNotebook.Tab", background="lightgray", fg="blue",
        ...
        self.edm_preview_text = scrolledtext.ScrolledText(self.siedm, background="#FCE6C9", font=("Arial", 16, "bold"),
                                        height=15, width=65, wrap=tk.WORD, fg="Black")
    ...
    def show_satellite_edm_preview(self, subsystem_sn):
        ...
        if fetched_data:
            ...
            self.edm_preview_text.insert(tk.END, f"||\tComments\t\t:   {fetched_data[7]}\n")
            self.edm_preview_text.insert(tk.END, f"||\tReport Link\t\t:   {fetched_data[8]}\n")
            self.edm_preview_text.insert(tk.END,
                                         f"||________________________________________________________________\n")
            self.edm_preview_text.insert(tk.END,
                                         f"||________________________________________________________________||\n")
